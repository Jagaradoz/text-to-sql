import { create } from 'zustand';
import { persist } from 'zustand/middleware';
import axios from 'axios';
import { fetchDatabaseOverview, generate } from '@/lib/api';
import { AppState } from '@/types/store';
import { GenerateParams } from '@/types/api';

function getErrorMessage(err: unknown, fallback: string) {
  if (axios.isAxiosError(err) && err.response?.data?.detail) {
    return err.response.data.detail;
  }
  if (err instanceof Error && err.message) {
    return err.message;
  }
  return fallback;
}

export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      databases: [],
      activeResult: null,
      isLoading: false,
      error: null,
      
      // Configuration
      apiKey: '',
      provider: 'google',
      activeModel: 'gemini-1.5-flash',

      resetError: () => set({ error: null }),
      setApiKey: (key: string) => set({ apiKey: key }),
      setProvider: (provider: "openai" | "google") => set({ provider }),
      setActiveModel: (activeModel: string) => set({ activeModel }),

      getDatabases: async () => {
        set({ isLoading: true });
        try {
          const data = await fetchDatabaseOverview();
          set({ databases: data.databases, isLoading: false });
        } catch (err: unknown) {
          console.warn('Could not fetch databases:', getErrorMessage(err, 'Unknown error'));
          set({ databases: [], isLoading: false });
        }
      },

      submitGenerate: async (params: GenerateParams) => {
        set({ isLoading: true, error: null });
        try {
          // Merge store config into request params
          const requestParams: GenerateParams = {
            ...params,
            provider: get().provider,
            model_name: get().activeModel,
          };
          
          const data = await generate(requestParams, get().apiKey || undefined);
          set({ activeResult: data, isLoading: false });
        } catch (err: unknown) {
          set({ error: getErrorMessage(err, 'Failed to generate'), isLoading: false });
        }
      },
    }),
    {
      name: 'text-to-sql-storage',
      partialize: (state) => ({
        apiKey: state.apiKey,
        provider: state.provider,
        activeModel: state.activeModel,
      }),
    }
  )
);
